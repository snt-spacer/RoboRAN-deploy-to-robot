...
class GoToPoseFormater(Registerable, BaseFormater):
    ...
    def build_logs(self):
        ...
        self._logs["target_heading_error"] = 0
    ...
    def update_goal(self, position: torch.Tensor | None = None, heading: torch.Tensor | None = None, **kwargs):
        if position is not None:
            self._step += 1
            self._target_position[0] = position[:, :2]
        if heading is not None:
            self._step += 1
            self._target_heading[0] = heading
    
    def update_goal_ROS(self, position: PoseStamped | None = None, **kwargs):
        if position is not None:
            self._step += 1
            self._target_position[0, 0] = position.pose.position.x
            self._target_position[0, 1] = position.pose.position.y
            self._target_heading[0, 0] = torch.atan2(
                2 * (position.pose.orientation.w * position.pose.orientation.z + position.pose.orientation.x * position.pose.orientation.y),
                1 - 2 * (position.pose.orientation.y ** 2 + position.pose.orientation.z ** 2),
            )
