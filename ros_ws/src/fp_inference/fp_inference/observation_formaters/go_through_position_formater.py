...
class GoThroughPositionFormater(Registerable, BaseFormater):
    ...
    def __init__(
        ...
        device: str = "cuda",
        max_steps: int = 500,
        task_cfg: GoThroughPositionTaskCfg = GoThroughPositionTaskCfg(),
        num_actions: int = 2,
        **kwargs,
    ) -> None:
    ...
    def format_observation(self, actions: torch.Tensor | None = None) -> None:
        ...
        self._task_data[:, 3] = torch.cos(self.target_heading_error)
        self._task_data[:, 4] = torch.sin(self.target_heading_error)
        self._task_data[0, 5] = self.dist
        self._observation = torch.cat((actions, self._task_data), dim=1)
