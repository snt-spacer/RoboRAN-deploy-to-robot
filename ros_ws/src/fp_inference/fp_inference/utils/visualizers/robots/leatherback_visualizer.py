...
class LeatherbackVisualizer(BaseRobotVisualizer, Registerable):
    ...
    def compute_energy(self) -> None:
        ...
        dt[0] = dt[1] 
        # Compute energy
        energy = (self._data['commands.throttle.m/s']) * dt
        return energy
    ...
    def plot_actions(self) -> None:
        ...
        ax.plot(self._data['elapsed_time.s'], self._data['actions.throttle'], label='Linear Velocity Cmd')
        ax.plot(self._data['elapsed_time.s'], self._data['actions.steering_angle'], label='Steering Angle Cmd')
        ax.set_ylim(-1, 1)
    ...
    def plot_commands(self):
        fig = plt.figure(figsize=(10,5))
        ax = fig.add_subplot(1, 1, 1)
        ax.plot(self._data['elasped_time.s'], self._data['commands.throttle.m/s'], label='Linear Velocity Cmd')
        ax.plot(self._data['elapsed_time.s'], self._data['commands.steering_angle.rad'], label='Steering Angle Cmd')
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('Command')
        ax.set_title('Robot Commands')
        ax.legend()
