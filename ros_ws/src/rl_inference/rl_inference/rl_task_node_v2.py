...
from .observation_formaters.base_formater import BaseFormaterCfg
...
class RLTaskNode(Node):
    ...
    def __init__(self) -> None:
        ...
        task_name_desc = ParameterDescriptor(
            description='The name of the task to be executed.\
                ...
                ", ".join(ObservationFormaterFactory._registry.keys())
            )
        )
        ...
        state_preprocessor_name_desc = ParameterDescriptor(
            description='The name of the state preprocessor to be used.\
                ...
                ", ".join(StatePreprocessorFactory._registry.keys())
            )
        )
        ...
        robot_interface_name_desc = ParameterDescriptor(
            description='The name of the robot interface to be used.\
                ...
                ", ".join(RobotInterfaceFactory._registry.keys())
            )
        )
        ...
        inference_runner_name_desc = ParameterDescriptor(
            description='The name of the inference runner to be used.\
                ...
                ", ".join(InferenceRunnerFactory._registry.keys())
            )
        )
        ...
        max_run_time_desc = ParameterDescriptor(description="The maximum run time of the task in seconds.")
        self.declare_parameter("max_run_time", 30, max_run_time_desc)
        self._max_run_time = self.get_parameter("max_run_time").get_parameter_value().integer_value
        ...
        self._max_steps = int(self._max_run_time / self._dt)
        enable_plotting_desc = ParameterDescriptor(description="Enable plotting of the task execution.")
        self.declare_parameter("enable_plotting", False, enable_plotting_desc)
        self._enable_plotting = self.get_parameter("enable_plotting").get_parameter_value().bool_value
    ...
    def build(self) -> None:
        ...
        self._data_logger = Logger(objects, self._enable_logging, self._logs_save_path, generate_plots=self._enable_plotting)
