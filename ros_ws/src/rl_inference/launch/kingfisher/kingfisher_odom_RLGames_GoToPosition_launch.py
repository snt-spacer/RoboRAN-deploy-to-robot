...
def generate_launch_description():
    ...
    task_name = LaunchConfiguration("task_name", default="GoToPosition")
    state_preprocessor_name = LaunchConfiguration("state_preprocessor_name", default="Odometry")
    robot_interface_name = LaunchConfiguration("robot_interface_name", default="Kingfisher")
    inference_runner_name = LaunchConfiguration("inference_runner_name", default="RLGames")
    enable_logging = LaunchConfiguration("enable_logging", default=True)
    enable_plotting = LaunchConfiguration("enable_plotting", default=False)
    device = LaunchConfiguration("device", default="cpu")
    max_run_time = LaunchConfiguration("max_run_time", default=30)
    dt = LaunchConfiguration("dt", default=1 / 12.5)
    terminate_on_completion = LaunchConfiguration("terminate_on_completion", default=False)
    nn_checkpoint_path = LaunchConfiguration("nn_checkpoint_path", default="/RANS_DeployToRobot/models/RLGames/GoToPosition/2025-02-22_17-12-34/nn/kingfisher_direct.pth")
    nn_log_dir = LaunchConfiguration("nn_log_dir", default="/RANS_DeployToRobot/models/RLGames/GoToPosition/2025-02-22_17-12-34/")
    ...
    nn_checkpoint_path_arg = DeclareLaunchArgument(
        "nn_checkpoint_path",
        default_value="/RANS_DeployToRobot/models/RLGames/GoToPosition/2025-02-22_17-12-34/nn/kingfisher_direct.pth",
        description="The path to the neural network checkpoint.",
    )
    nn_log_dir_arg = DeclareLaunchArgument(
        "nn_log_dir",
        default_value="/RANS_DeployToRobot/models/RLGames/GoToPosition/2025-02-22_17-12-34/",
